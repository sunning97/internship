    ...
    public function bubbleDown($index)
    {
        ...
        while ($index < (int)($this->_current_Size/2)) {
            ...
            if($this->_type == 'address') {
                if ($rightChild < $this->_current_Size && (int)substr($this->heap_Array[$leftChild]->getKey()['address'], 0, strpos($this->heap_Array[$leftChild]->getKey()['address'], ' ')) < (int)substr($this->heap_Array[$rightChild]->getKey()['address'], 0, strpos($this->heap_Array[$rightChild]->getKey()['address'], ' '))) // right child exists?
                {
                ...
                if ((int)substr($top->getKey()['address'], 0, strpos($top->getKey()['address'], ' ')) >= (int)substr($this->heap_Array[$larger_Child]->getKey()['address'], 0, strpos($this->heap_Array[$larger_Child]->getKey()['address'], ' '))) {
                    break;
                }
