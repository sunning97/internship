        ...
        if (!isset($_SESSION['login'])) {
            ...
            if ($lands == null) {
                ...
                foreach ($owners as $item) {
                    $excel->getActiveSheet()->setCellValue('A' . $numRow, $item['owner_name']);
                    $excel->getActiveSheet()->setCellValue('B' . $numRow, $item['owner_email']);
                    $excel->getActiveSheet()->setCellValue('C' . $numRow, $item['owner_phone']);
                    $numRow++;
                }
    ...
    private function getDataFromFIle()
    {
        ...
        if (in_array($ext, $allow) === false) {
            ...
            foreach ($data_owners as $data) {
                if ($ownerModel->search($data[0], $data[1]) == null) {
            ...
            foreach ($arr as $value) {
                ...
                foreach ($data_owners as $owner) {
                    if ($owner[0] == $value->getOwner()) {
                        $own = $ownerModel->search($value->getOwner(), $owner[1]);
                    }
                }
